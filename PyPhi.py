...
def marquee2(s, x, y, xWrap):
    lcd.setCursor(0, y)
    if x > 0: # Initially scrolls in from right edge
        lcd.message(' ' * x + s[0:16-x])
    else:     # Then scrolls w/wrap indefinitely
        lcd.message(s[0:16-x])
    if x < xWrap: return 0
    return x - 1
...
while pianobar.isalive():
    ...
    if not staSel:
        ...
        if songTitle is not None:
            xTitle = marquee(songTitle, xTitle, 0, xTitleWrap)        # Integerize current and new volume values
        volCurI = int((volCur - VOL_MIN) + 0.5)
        ...
        if volSet:
            ...
            if (time.time() - playMsgTime) >= 3:
                ...
                xInfo = marquee2(songInfo, xInfo, 1, xInfoWrap)
